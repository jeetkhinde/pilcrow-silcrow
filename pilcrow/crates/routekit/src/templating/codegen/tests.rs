...
"#;
    ...
    fn static_page_template_error_uses_html_response() {
        ...
        // The tracing::error! log still says "template render failed" — that's fine.
        // What must NOT appear is a plain-text 500 response tuple using that string.
        assert!(
            !source.contains("INTERNAL_SERVER_ERROR, \"template render failed\""),
            "plain-text error response tuple must not appear in generated source:\n{source}"
        );
        assert!(
