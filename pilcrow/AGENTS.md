...
pub const PROMOTE_AFTER: u32 = 0;          // FSR bake threshold (0 = bake on first hit)
```

- **`PROMOTE_AFTER`**: FSR bake threshold. `0` = bake on first hit. See FSR section.
- **Removed (build errors):** `PRERENDER`, `REVALIDATE`, `STREAMING`, `CACHE_TAGS`, `MAX_STALE`, `CACHE_VARY` — using any of these now produces a build error. Migration: `PRERENDER → pub const PROMOTE_AFTER: u32 = 0`, `REVALIDATE → #[revalidate(N)]` on a `LiveProp` field.
