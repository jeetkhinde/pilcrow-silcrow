...
function connectSseHub(hub) {
  ...
  es.addEventListener("live", function (e) {
    try {
      ...
      // ⚡ Bolt: Single DOM pass to update both live text fields and live mods
...
function publishOptimistic(scope, data, mutationId) {
  ...
  const liveSnapshots = {};
  // ⚡ Bolt: Single DOM pass to update optimistic fields, avoiding O(N) DOM scans
...
function revertOptimistic(mutationId) {
  ...
  // ⚡ Bolt: Single DOM pass to revert optimistic fields, avoiding O(N) DOM scans
