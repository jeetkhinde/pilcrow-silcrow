...
function connectSseHub(hub) {
  ...
  es.addEventListener("live", function (e) {
    try {
      ...
      // ⚡ Bolt Optimization: Consolidate sequential DOM passes into a single scan.
      // querySelectorAll is synchronous and blocks the thread; scanning the DOM once
      // with a combined selector is significantly faster for updates.
...
function publishOptimistic(scope, data, mutationId) {
  ...
  // ⚡ Bolt Optimization: Single DOM pass. Replaces O(keys) querySelectorAll calls
  // with a single O(1) querySelectorAll pass to eliminate repetitive tree traversal.
  document.querySelectorAll("[data-pilcrow-live-field]").forEach(function(n) {
    const k = n.getAttribute("data-pilcrow-live-field");
    if (k in data) {
      if (!(k in liveSnapshots)) liveSnapshots[k] = n.textContent;
      n.textContent = data[k] == null ? "" : String(data[k]);
    }
  });
...
function revertOptimistic(mutationId) {
  ...
  // ⚡ Bolt Optimization: Replace O(keys) querySelectorAll calls
  // with a single pass to revert nodes.
  if (entry.liveSnapshots && Object.keys(entry.liveSnapshots).length > 0) {
    document.querySelectorAll("[data-pilcrow-live-field]").forEach(function(n) {
      const k = n.getAttribute("data-pilcrow-live-field");
