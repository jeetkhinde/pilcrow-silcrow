...
function publishOptimistic(scope, data, mutationId) {
  ...
  // PERFORMANCE: Avoid O(n) DOM scans. Query the DOM once and filter during iteration.
  document.querySelectorAll("[data-pilcrow-live-field]").forEach(function(n) {
    const k = n.getAttribute("data-pilcrow-live-field");
    if (k in data) {
...
function revertOptimistic(mutationId) {
  ...
  if (Object.keys(liveSnapshots).length > 0) {
    // PERFORMANCE: Avoid O(n) DOM scans during revert. Query the DOM once and filter during iteration.
    document.querySelectorAll("[data-pilcrow-live-field]").forEach(function(n) {
      const k = n.getAttribute("data-pilcrow-live-field");
