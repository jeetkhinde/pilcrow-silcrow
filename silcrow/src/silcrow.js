...
function publishOptimistic(scope, data, mutationId) {
  ...
  const liveSnapshots = {};
  // ⚡ Bolt: Consolidate repeated DOM queries for each updated key into a single pass
  document.querySelectorAll("[data-pilcrow-live-field]").forEach(function (n) {
    const k = n.getAttribute("data-pilcrow-live-field");
    if (k in data) {
...
function revertOptimistic(mutationId) {
  ...
  // ⚡ Bolt: Consolidate repeated DOM queries for each reverted key into a single pass
  document.querySelectorAll("[data-pilcrow-live-field]").forEach(function(n) {
    const k = n.getAttribute("data-pilcrow-live-field");
    if (k in liveSnaps) {
