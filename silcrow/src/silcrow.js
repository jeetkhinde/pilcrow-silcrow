...
function connectSseHub(hub) {
  ...
  es.addEventListener("live", function (e) {
    try {
      ...
      // ⚡ Bolt: Consolidated sequential document scans for live fields and modifiers into a single pass
...
function publishOptimistic(scope, data, mutationId) {
  ...
  const liveSnapshots = {};
  // ⚡ Bolt: Consolidated O(N) querySelectorAll calls in loop into a single O(1) document scan
