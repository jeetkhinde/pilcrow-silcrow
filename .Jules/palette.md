...
## 2024-06-15 - Restore Keyboard Focus Outlines Overridden by Tailwind Preflight
**Learning:** Tailwind CSS's default Preflight completely strips `outline` from all elements. If you use interactive elements (links, buttons, inputs) in this app without explicitly adding `:focus-visible` styles, keyboard users will have no visual indicator of where their focus is.
**Action:** Always verify that every interactive element has explicit focus ring utility classes (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2`) when working with Tailwind in this codebase to ensure keyboard accessibility.
