...
import { useSilcrowAtom, publishSilcrowAtom, useSilcrowPrefetch, useSilcrowRoute, useSilcrowAction, usePilcrowNamedAction, useSilcrowResource, useSilcrowForm, useSilcrowMutation, PilcrowReactProvider, resolvePilcrowAction, } from "./hooks.js";
...
async function runTests() {
    ...
    {
        function TestRoute() {
            const cart = useSilcrowRoute("/cart", { count: 0 });
    ...
    {
        const action = submitSilcrow("/cart/add", { scope: "cart:add" });
    ...
    {
        const handler = silcrowSubmitHandler("/cart/update", { method: "PATCH" });
    ...
    {
        function TestAction() {
            const [state, action, pending] = useSilcrowAction("/action-url", { val: "initial" });
    ...
    // 10. usePilcrowNamedAction
    {
        function TestNamedAction() {
            const [state] = usePilcrowNamedAction("testAction", { val: "named-initial" });
            return <span>{state.val}</span>;
        }
        const html = renderToStaticMarkup(<PilcrowReactProvider value={{ actionBase: "/context-base" }}>
        <TestNamedAction />
    ...
    // 12. useSilcrowMutation
    {
        ...
        function TestMutation() {
            ...
            globalThis.triggerMutate = mut.mutate;
            return (<div>
        ...
        const promise = globalThis.triggerMutate({ item: 999 });
        await promise;
