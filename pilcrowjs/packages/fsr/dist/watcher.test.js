...
async function runTests() {
    ...
    try {
        ...
        await store.upsertSlot(route, 'test_slot', 'SELECT val FROM watcher_test_dummy WHERE id = $1', [1], ['watcher_dep_key'], 0, // no debounce
        'val');
        // 2. Setup watcher
        const config = {
            pollIntervalMs: 200,
        ...
        const patches = [];
        watcher.getEmitter().on('patch', (patch) => {
            patches.push(patch);
        });
        await watcher.start();
        ...
        await store.upsertSlot(route, 'test_slot', 'SELECT val FROM watcher_test_dummy WHERE id = $1', [1], ['scheduled_dep'], 0, 'val');
