...
function needsCsrfCheck(method, headers) {
    if (!['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
        return false;
    }
    const ct = headers.get('content-type') || '';
    return FORM_CONTENT_TYPES.some((allowed) => ct.trim().toLowerCase().startsWith(allowed));
}
...
export const csrf = () => (app) => app.onBeforeHandle(({ request, set }) => {
