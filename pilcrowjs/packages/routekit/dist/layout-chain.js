 ...
 */
export function composeLayoutChain(react, PageComponent, layouts, pagePattern, props) {
    ...
    currentElement = react.createElement('div', { 'data-ps-layout': pagePattern, style: { display: 'contents' } }, currentElement);
    ...
    for (let i = layouts.length - 1; i >= 0; i--) {
        ...
        const slotElement = react.createElement('div', { 'data-ps-slot': childPattern, style: { display: 'contents' } }, currentElement);
        // Instantiate layout with the slot element as children
        const layoutElement = react.createElement(LayoutComponent, props, slotElement);
        // Wrap layout in its own layout container if not outermost (i > 0)
        if (i > 0) {
            currentElement = react.createElement('div', { 'data-ps-layout': layoutPattern, style: { display: 'contents' } }, layoutElement);
        }
        else {
...
//# sourceMappingURL=layout-chain.js.map